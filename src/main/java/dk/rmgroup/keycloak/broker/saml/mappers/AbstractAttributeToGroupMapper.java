...
/**
 * Abstract class that handles the logic for importing and updating brokered
 * users for all mappers that map a SAML
...
public abstract class AbstractAttributeToGroupMapper extends AbstractIdentityProviderMapper {
     ...
     * {@code true} if their mapping can be applied
     * (i.e. user has the SAML attribute that should be mapped) or {@code false}
     * otherwise.
     *
     ...
     * @return {@code true} if the mapping can be applied or {@code false}
     *         otherwise.
     */
     ...
     */
    private GroupModel getGroup(KeycloakSession session, final RealmModel realm,
            final IdentityProviderMapperModel mapperModel) {
        GroupModel group = KeycloakModelUtils.findGroupByPath(session, realm,
                mapperModel.getConfig().get(ConfigConstants.GROUP));

        if (group == null) {
            throw new IdentityBrokerException("Unable to find group: " + mapperModel.getConfig().get(ConfigConstants.GROUP));
        }
        return group;
    }
}
